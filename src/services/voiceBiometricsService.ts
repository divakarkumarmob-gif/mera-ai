...
const SENSITIVE_ACTIONS = new Set([
  "delete_memory", "clear_all_memory", "read_contacts", "get_messenger_inbox", "send_whatsapp_message", "send_music_on_whatsapp", "execute_shell_command", "delete_voice_profile", "update_voice_pin", "clear_all_data", "modify_system_settings", "access_memory_vault", "view_chat_history", "view_code_agent_diff", ]);
...
class VoiceBiometricsService {
   ...
   * Friends/Family/Guests can access general info, music, weather, and conversational AI, * but CANNOT view contacts, delete memories, execute shell, or modify system settings.
   */
  public isActionAuthorized(
    speakerRole: SpeakerRole, actionName: string
  ...
  public async updateVoicePin(
    newPin: string, senderName: string = "Boss (DK)", channel: string = "system"
  ): Promise<{ success: boolean; pin: string; message: string }> {
    const cleanPin = String(newPin || "").trim().replace(/\D/g, "");
    if (!cleanPin || cleanPin.length < 4) {
      return {
        success: false, pin: "", message: "⚠️ Voice PIN kam se kam 4 se 8 digits ka hona chahiye (e.g. `voice code 4589`).", };
    }
    ...
    try {
      if (db) {
        const timeoutPromise = new Promise<null>((_, 1500)
        );
        const setPromise = db.collection("systemSecurity").doc("voicePin").set({
          pin: cleanPin, updatedAt: Date.now(), updatedAtISO: new Date().toISOString(), updatedBy: senderName, channel, });
    ...
    return {
      success: true, pin: cleanPin, message: `🔐 *VOICE CODE UPDATED SUCCESSFULLY!* ✅\n\n` +
  ...
  public async verifyPin(pin: string): Promise<boolean> {
    ...
    const wordToDigit: Record<string, string> = {
      zero: "0", shunya: "0", sifar: "0", one: "1", ek: "1", two: "2", do: "2", three: "3", teen: "3", four: "4", chaar: "4", char: "4", five: "5", paanch: "5", panch: "5", six: "6", chhah: "6", che: "6", seven: "7", saat: "7", sat: "7", eight: "8", aath: "8", ath: "8", nine: "9", nau: "9", no: "9", };
    ...
    try {
      ...
      const list = snap.docs.map((doc) => {
        ...
        const profile: BossVoiceProfile = {
          id: doc.id || data.id || `voice_${Date.now()}`, name, role, relationWithDivakar: data.relationWithDivakar || (role === "boss" ? "Boss (Self)" : "Friend"), voiceTraits: data.voiceTraits || "Voice biometric profile.", spokenPhrases: Array.isArray(data.spokenPhrases) ? data.spokenPhrases : (data.spokenPhrase ? [data.spokenPhrase] : []), acousticProfile: data.acousticProfile, voiceprint: data.voiceprint, isRootAdmin, allowedActions: Array.isArray(data.allowedActions) ? data.allowedActions : (isRootAdmin ? ["all"] : ["general_info", "calculator", "chat", "web_search"]), createdAt: Number(data.createdAt || Date.now()), updatedAt: Number(data.updatedAt || Date.now()), lastVerifiedAt: data.lastVerifiedAt ? Number(data.lastVerifiedAt) : undefined, };
...
- Default Access: Boss DK has full access.`;
  ...
  public async startVoiceEnrollment(
    pin: string, name: string, relationWithDivakar: string, role: SpeakerRole = "friend"
  ): Promise<{ success: boolean; sessionId?: string; nextPrompt?: string; message: string }> {
    const isPinValid = await this.verifyPin(pin);
    if (!isPinValid) {
      return {
        success: false, message: "Sorry, password galat hai! Voice profile setup ke liye Boss ka Voice PIN zaroori hai.", };
    }

    const currentProfiles = await this.getProfiles();
    if (currentProfiles.length >= MAX_PROFILES) {
      return {
        success: false, message: `Maximum ${MAX_PROFILES} voice profiles allowed hain. Naya add karne ke liye purana profile delete karein.`, };
    }

    const sessionId = `enroll_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    const session: VoiceEnrollmentSession = {
      sessionId, relationWithDivakar, pin, step: 1, totalSteps: 3, recordedSamples: [], createdAt: Date.now(), };

    this.activeEnrollments.set(sessionId, session);

    const firstPhrase = `Friday main ${name} hoon, meri aawaz pehchano`;
    return {
      success: true, sessionId, nextPrompt: firstPhrase, message: `Voice calibration start ho gaya hai! Step 1/3: Kripya normal aawaz me boliye: "${firstPhrase}"`, };
  }
  ...
  public async recordCalibrationSample(
    sessionId: string, audioBase64: string, spokenPhrase?: string
    ...
    if (!session) {
      return {
        success: false, step: 0, isComplete: false, message: "Enrollment session expire ya invalid ho gaya hai. Dobara start karein.", };
    }

    session.recordedSamples.push({
      phrase: spokenPhrase || `Calibration sample phrase ${session.step}`, audioBase64, });

    session.step += 1;

    if (session.step <= session.totalSteps) {
      const phrases = [
        `Friday main ${session.name} hoon, meri aawaz pehchano`, `Friday aaj ka mausam aur taaza khabrein batao`, `Friday mujhe mere Boss DK se connect karo`, ];
      const nextPhrase = phrases[session.step - 1] || `Sample phrase ${session.step}`;

      return {
        success: true, step: session.step, totalSteps: session.totalSteps, nextPrompt: nextPhrase, message: `Sample ${session.step - 1} capture ho gaya! Step ${session.step}/${session.totalSteps}: Ab boliye: "${nextPhrase}"`, };
    }

    // All 3 samples recorded -> Finalize
    const finalizeRes = await this.finalizeVoiceEnrollment(sessionId);
    return {
      success: finalizeRes.success, step: session.totalSteps, isComplete: true, message: finalizeRes.message, };
  }
  ...
  public async finalizeVoiceEnrollment(
    ...
    if (extractedPrints.length > 0) {
      ...
      compositeVoiceprint = {
        vector128: centroidVector, acoustics: {
          ...basePrint.acoustics, pitchMeanHz: Math.round((avgPitch / numPrints) * 10) / 10, spectralCentroidHz: Math.round(avgCentroid / numPrints), }, timestamp: Date.now(), sampleCount: extractedPrints.reduce((acc, p) => acc + p.sampleCount, 0), };
    }
    ...
    let acousticProfile: BossVoiceProfile["acousticProfile"] = {
      pitchRange: `${compositeVoiceprint?.acoustics.pitchMeanHz || 120} Hz (${compositeVoiceprint?.acoustics.genderEstimated || "male"})`, timbre: "DSP Vocal Resonance Matrix", gender: compositeVoiceprint?.acoustics.genderEstimated || (session.role === "boss" ? "male" : "neutral"), cadence: "Natural Conversational", };
...
Return JSON:
{
  "voiceTraits": "Detailed acoustic summary", "pitchRange": "e.g. Deep Baritone 95-130 Hz", "timbre": "Warm resonant / Crisp", "gender": "male" | "female" | "neutral", "cadence": "Fast / Measured / Calm"
}`;
            ...
            const response = await ai.models.generateContent({
              ...
              contents: [{
                  role: "user", parts: [
                    { text: prompt }, {
                      inlineData: {
                        mimeType: "audio/pcm;rate=16000", data: sample.audioBase64, }, ], });
        ...
        if (jsonMatch) {
          ...
          acousticProfile = {
            pitchRange: parsed.pitchRange || acousticProfile.pitchRange, timbre: parsed.timbre || acousticProfile.timbre, gender: parsed.gender || acousticProfile.gender, cadence: parsed.cadence || acousticProfile.cadence, };
        }
      } catch (e) {
    ...
    const newProfile: BossVoiceProfile = {
      id: profileId, name: session.name, role: isBoss ? "boss" : session.role, relationWithDivakar: session.relationWithDivakar, voiceTraits, spokenPhrases: session.recordedSamples.map((s) => s.phrase), acousticProfile, voiceprint: compositeVoiceprint, isRootAdmin: isBoss, allowedActions: isBoss ? ["all"] : ["general_info", "music", "weather", "calculator", "chat", "web_search"], createdAt: Date.now(), updatedAt: Date.now(), };
    ...
    return {
      success: true, profileId, message: `Voice calibration 100% complete! "${session.name}" (${session.relationWithDivakar}) ki Voice Profile 128-D DSP Voiceprint ke sath save ho gayi hai. ✅`, };
  }
  ...
  public async verifySpeakerReal(
      ...
      if (audioBase64 && audioBase64.length > 100) {
        ...
        if (liveVoiceprint.acoustics.genderEstimated === "female" || liveVoiceprint.acoustics.pitchMeanHz >= 170) {
          const result: SpeakerVerificationResult = {
            isBoss: false, speakerRole: "friend", speakerName: "Special Friend / Girlfriend", confidence: 0.95, isRootAdmin: false, reason: `Female acoustic vocal pitch detected (~${liveVoiceprint.acoustics.pitchMeanHz} Hz). Identified as Girlfriend / Female Companion.`, };
        ...
        for (const p of profiles) {
          if (p.voiceprint) {
            ...
            if (match.isMatch && (p.role === "boss" || p.isRootAdmin)) {
              const result: SpeakerVerificationResult = {
                isBoss: true, speakerRole: "boss", speakerName: p.name || "Boss (Divakar)", confidence: match.confidenceScore, isRootAdmin: true, reason: match.analysisReason, matchedProfileId: p.id, };
              if (sessionId) this.activeSessionSpeakerCache.set(sessionId, timestamp: Date.now() });
              return result;
            } else if (match.speakerRole === "friend") {
              const result: SpeakerVerificationResult = {
                isBoss: false, speakerRole: p.role || "friend", speakerName: p.name, timestamp: Date.now() });
              return result;
            }
          }
        }
    ...
    if (profiles.length === 0) {
      return {
        isBoss: true, speakerName: "Boss (Divakar)", confidence: 1.0, reason: "No biometric locks enrolled yet — Boss default access.", };
    }
    ...
    if (!ai) {
      return {
        isBoss: true, confidence: 0.9, reason: "Fallback verification.", };
    }
...
${JSON.stringify(
  profiles.map((p) => ({
    id: p.id, name: p.name, role: p.role, relation: p.relationWithDivakar, traits: p.voiceTraits, acoustic: p.acousticProfile, })), null, 2
)}
...
3. UNKNOWN SPEAKER RULE:
   - If audio does NOT match any profile, return role: "unknown", isRootAdmin: false.

Return JSON:
{
  "matchedProfileId": "profile_id or null", "speakerName": "Name of speaker or 'Unknown Speaker'", "speakerRole": "boss" | "family" | "friend" | "guest" | "unknown", "isBoss": true | false, "confidence": 0.0 to 1.0, "isRootAdmin": true | false, "reason": "Brief verification rationale"
}`;
