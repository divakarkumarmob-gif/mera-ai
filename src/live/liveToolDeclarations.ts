...
const rawFunctionDeclarations: any[] = [
  ...
  {
    "name": "avatar_action",
    "description": "AVATAR BODY ACTION TOOL: Friday has a visible 3D body on screen. Call this when DK asks her to DO physical body actions. CRITICAL: Follow DK's exact orders! If DK specifies ANY custom sequence in ANY order: pass all in 'sequence' array OR comma-separated in 'action'. For single action: pass 'action'. 'dance karo' -> action:'dance' triggers full choreography show. RANDOM VARIANTS: flip, jump, flip_kick, dance_silly, sad each pick randomly from multiple animations. Available actions: dance, dance_salsa, dance_swing, dance_silly, dance_robot, dance_samba, dance_breakfreeze, rap, flip, flip_front, flip_uppercut, flip_twist, flip_kick, run_flip, jump, big_jump, jumping_jacks, pushup, situps, bicycle_crunch, arm_stretch, warming_up, warmup (random warm-up), workout (random workout), salute, namaste, wave, bow, phone, nod-yes, nod-no, walk, excited, victory, flair, blow_kiss, angry, sad, dying, fist_fight, kicking, punching, fighting_idle, think, stop. Always speak enthusiastic Hinglish confirmation.",
    "parameters": {
      "type": "OBJECT",
      "properties": {
        "action": {
